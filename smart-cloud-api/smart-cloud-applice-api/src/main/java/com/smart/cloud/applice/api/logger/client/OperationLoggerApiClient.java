package com.smart.cloud.applice.api.logger.client;


import com.smart.cloud.common.api.service.client.ApiClient;

/**
 * @className: com.smart.cloud.applice.api.logger.client.OperationLoggerApiClient
 * @projectName: 封装SmartCloud项目-OperationLoggerApiClient类
 * @module: SmartCloud项目-OperationLoggerApiClient类，主要位于OperationLogger-操作日志ApiClient模块的业务场景
 * @content: OperationLoggerApiClient类，主要用于完成应用操作日志API客户端的封装和定义.
 * @author: Powered by Marklin
 * @datetime: 2023-06-22 05:06
 * @version: 1.0.0
 * @copyright: Copyright © 2018-2023 SmartCloud Systems Incorporated. All rights reserved.
 */
public interface OperationLoggerApiClient extends ApiClient {
}
